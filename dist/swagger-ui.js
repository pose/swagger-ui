...
(function() {
  var ContentTypeView, HeaderView, MainView, OAuth2Client, OperationView, ParameterView, ResourceView, SignatureView, StatusCodeView, SwaggerUi,
  ...
  SwaggerUi = (function(_super) {
    ...
    SwaggerUi.prototype.render = function() {
      ...
      if ((this.api.authorization != null) && __indexOf.call(this.api.authorization.grantTypes, "IMPLICIT") >= 0 && this.options.oauth2) {
        this.options.oauth2.register(this.api.authorization.scopes, this.api.authorization.authorizationEndpoint);
      }
      this.mainView = new MainView({
        model: this.api,
        el: $('#' + this.dom_id),
        oauth2: this.options.oauth2
      }).render();
      this.showMessage();
  ...
  OAuth2Client = (function() {
    ...
    Token = (function() {
      ...
      function Token(accessToken, tokenType, expiresIn, state) {
    ...
    OAuth2Client.prototype.timeout = null;

    OAuth2Client.prototype.scopes = [];

    OAuth2Client.prototype.authorizationEndpoint = null;

    function OAuth2Client(options) {
      var defaultOptions, key, mandatoryKeys, result, value, _i, _len;
      ...
      for (_i = 0, _len = mandatoryKeys.length; _i < _len; _i++) {
        key = mandatoryKeys[_i];
        if (this.options[key] === void 0) {
          throw new Error("" + key + " parameter of oauth2 must be specified");
        }
      }
      result = this.parseHashQueryString(window.location.hash);
      if ((result[0].accessToken != null) || (result[1].message != null)) {
        localStorage['returnValue'] = JSON.stringify(result);
        window.close();
      }
    }

    OAuth2Client.prototype.unauthorize = function() {
      return this.options.onUnauthorized();
    };

    OAuth2Client.prototype.register = function(scopes, authorizationEndpoint) {
      this.scopes = scopes;
      return this.authorizationEndpoint = authorizationEndpoint;
    };

    OAuth2Client.prototype.login = function() {
      var authUrl, error, key, popUpToken, query_params, query_string, retVal, value, _base, _base1, _ref,
        _this = this;
      query_params = {
        response_type: 'token',
      ...
      authUrl = "" + this.authorizationEndpoint + "?" + query_string;
      this.popupWindow(authUrl, 'Authorize', 1024, 800);
      retVal = localStorage['returnValue'];
      if (retVal != null) {
        _ref = JSON.parse(retVal), popUpToken = _ref[0], error = _ref[1];
        delete localStorage['returnValue'];
      }
      if (error.message != null) {
        return typeof (_base = this.options).onFailure === "function" ? _base.onFailure(error) : void 0;
      } else if (popUpToken.accessToken != null) {
        this.token = popUpToken;
        this.timeout = setTimeout(function() {
          var _base1;
          if (typeof (_base1 = _this.options).onTokenExpired === "function") {
            _base1.onTokenExpired(popUpToken);
          }
          return _this.logout();
        }, this.token.expiresIn * 1000);
        return typeof (_base1 = this.options).onComplete === "function" ? _base1.onComplete(popUpToken) : void 0;
      }
    };

    OAuth2Client.prototype.logout = function() {
      this.token = null;
      return clearTimeout(this.timeout);
    };

    OAuth2Client.prototype.parseHashQueryString = function(hash) {
      var dic, entries, entry, tuple, _i, _len, _ref;
      hash = hash.slice(1);
      entries = (function() {
      ...
      for (_i = 0, _len = entries.length; _i < _len; _i++) {
        ...
        if ((tuple[0] != null) && tuple[0] !== '') {
          dic[tuple[0]] = (_ref = tuple[1]) != null ? _ref : true;
        }
      }
      return [new Token(dic['access_token'], dic['token_type'], dic['expires_in'], dic['state']), new Error(dic['error'], dic['error_description'])];
    };

    OAuth2Client.prototype.popupWindow = function(url, title, w, h) {
      var key, left, options, properties, toText, top, value;
      ...
      options = ((function() {
        ...
        for (key in properties) {
          ...
          _results.push("" + key + ":" + (toText(value)));
        }
        return _results;
      })()).join('; ');
      return showModalDialog(url, null, options);
    };
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.submitOperation = function(e) {
      ...
      if (error_free) {
        ...
        if (this.options.oauth2 && (((_ref5 = this.options.oauth2) != null ? _ref5.token : void 0) != null)) {
          sep = invocationUrl.indexOf('?') !== -1 ? '&' : '?';
          invocationUrl += sep + ("access_token=" + this.options.oauth2.token.accessToken);
        } else if (this.model.authorization != null) {
          $('.submit').wiggle();
          if (this.options.oauth2) {
            this.options.oauth2.unauthorize();
          } else {
